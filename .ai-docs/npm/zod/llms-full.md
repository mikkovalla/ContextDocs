...
Note that using TypeScript's `enum` keyword is [not recommended](https://www.totaltypescript.com/why-i-dont-like-typescript-enums). </Callout>
...
If Zod can't find a discriminator key, it will throw an error at schema creation time. </Callout> \*/}
...
In Zod 3, exhaustiveness was not checked. To replicate the old behavior, use `z.partialRecord()`. </Callout>
...
{/\* <Callout>
Checks do not (in fact, cannot) change the inferred type of the schema. </Callout>
...
\<Tabs items={\['Zod', 'Zod Mini']}> <Tab value="Zod">
\`\`\`ts
...
The age of the user. Cannot be less than 0 </ParamField> \*/}
...
If you'd already implemented Zod 4 support according to the best practices described below (e.g. using the `"zod/v4/core"` subpath), then no other code changes should be necessary. This should not require a major version bump in your library. </Callout>
...
\<Tabs groupId="lib" items={\[ "Zod Mini", "Zod"]}> <Tab value="Zod Mini">
\`\`\`ts
